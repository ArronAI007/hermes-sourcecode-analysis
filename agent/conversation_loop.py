"""当前文件是 Hermes Agent 的核心对话循环，驱动一次完整用户交互的生命周期。
...
1. **Prologue（回合前准备）**：
   - stdio 守护、重试计数器重置、用户消息清洗
   - 系统提示词恢复/重建、会话持久化准备
   - 预压缩检查、`pre_llm_call` 插件钩子、外部记忆预取

2. **Main Loop（主循环）**：
   - 检查中断请求、消费迭代预算、触发 step_callback
   - 构建 `api_messages`（API 调用副本），注入预取记忆、MoA 上下文
   - 发起 API 调用（含重试、回退、流式处理）
   - 响应验证、finish_reason 检查、截断/内容策略/工具调用分支
...
- 编排多轮 API 调用，直到模型返回最终文本响应
- 处理流式响应（streaming），支持 TTS 等下游管线
- 分发工具调用，将执行结果注入上下文
- 错误分类与恢复：速率限制、上下文溢出、认证失败、内容策略拒绝
- 维护迭代预算、压缩状态、回退链、会话持久化

## 关键设计决策

- **API 消息与内部消息分离**：`messages`（权威会话历史）vs `api_messages`（每轮构建的 API 副本）
- **流式优先**：即使没有消费者，也使用流式路径获取健康检查（90s 陈旧流检测）
- **错误分类先行**：所有错误先通过 `classify_api_error()` 结构化分类，再决定恢复路径
- **宽限调用（Grace Call）**：预算耗尽后仍给最后一次机会
- **压缩是恢复手段而非预防手段**：只在检测到压力/溢出时触发
- **每轮重置计数器**：成功调用后重置各种重试计数器
- **空响应不立即放弃**：多层恢复（部分流恢复、上一轮内容回退、nudge、预填充、重试、fallback）
