...
``AIAgent.__init__`` 是代码库中最长的方法之一（60+ 参数，~1,400 行属性初始化、
提供商自动检测、凭证解析、上下文引擎引导等）。将其保留在 ``run_agent.py`` 中
会使该文件膨胀，而这些代码大多是"设置状态，然后忘记"。

提取后，主体代码以 ``init_agent(agent, ...)`` 的形式存在于此，
而 :meth:`AIAgent.__init__` 只是一个薄包装器，调用 ``init_agent(self, ...)``。
下面列出了模块加载时所需的所有导入；主体内部还进行了许多延迟导入。

测试在 ``run_agent.*`` 上打补丁的符号（``OpenAI``、``cleanup_vm`` 等）
通过 :func:`_ra` 解析，以保持补丁契约。
"""
