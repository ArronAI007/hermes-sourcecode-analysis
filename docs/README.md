...
| 文件 | 大小 | 职责 |
|------|------|------|
| `hermes_bootstrap.py` | 8.4KB | Windows UTF-8 修复、模块路径保护、懒加载激活 |
| `run_agent.py` | 268KB | **AIAgent 类**，核心编排器，处理对话循环和工具调度 |
| `cli.py` | 738KB | **CLI/TUI 界面**，命令行交互、REPL 循环 |
| `gateway/run.py` | 980KB | **网关运行时**，平台适配、会话调度 |
| `mcp_serve.py` | 33KB | MCP 服务端入口 |
| `batch_runner.py` | 57KB | 批量任务运行器 |
| `mini_swe_runner.py` | 28KB | 软件工程运行器 |

### 3.2 工具系统

| 文件 | 大小 | 职责 |
|------|------|------|
| `model_tools.py` | 58KB | 工具定义 Schema、工具调度入口 |
| `toolsets.py` | 35KB | 工具集分组、依赖检查 |
| `toolset_distributions.py` | 12KB | 工具集分发配置 |

### 3.3 Agent 核心

| 文件 | 大小 | 职责 |
|------|------|------|
| `agent/conversation_loop.py` | 298KB | **核心对话循环**，流式响应、工具调用处理 |
| `agent/chat_completion_helpers.py` | 149KB | LLM API 调用封装，多提供商适配 |
| `agent/context_compressor.py` | 149KB | 上下文压缩、摘要生成 |
| `agent/memory_manager.py` | ~100KB | 记忆管理、跨会话搜索 |
| `agent/curator.py` | 85KB | **学习系统**，自主技能创建和改进 |
| `agent/agent_init.py` | 100KB | Agent 初始化流程 |
| `agent/agent_runtime_helpers.py` | 144KB | 运行时辅助函数 |

### 3.4 支撑工具

| 文件 | 大小 | 职责 |
|------|------|------|
| `hermes_state.py` | 256KB | 状态管理、SQLite 数据库操作 |
| `hermes_constants.py` | 38KB | 常量定义、路径解析 |
| `hermes_logging.py` | 24KB | 日志配置和管理 |
| `utils.py` | 20KB | 通用工具函数 |
| `trajectory_compressor.py` | 69KB | 轨迹压缩，用于训练数据 |
