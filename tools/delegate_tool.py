#!/usr/bin/env python3
# =============================================================================
# delegate_tool.py - 子 Agent 委派工具
# =============================================================================
#
# 本模块实现 Hermes Agent 的子 Agent 委派功能，允许生成具有独立上下文的子 AIAgent 实例，
# 支持单任务和批量（并行）模式。父 Agent 会阻塞等待所有子任务完成。
#
# 子 Agent 获得的资源：
#   - 全新的对话（不继承父代历史）
#   - 独立的 task_id（独立的终端会话、文件操作缓存）
#   - 受限制的工具集（可配置，关键工具始终被剥离）
#   - 针对委派目标构建的专注系统提示
#
# 安全设计：
#   - 父代上下文只看到委派调用和汇总结果
