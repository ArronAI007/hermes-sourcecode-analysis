# =============================================================================
# gateway/delivery.py - 消息投送路由模块
# =============================================================================
#
# 本模块负责将 cron 任务输出和 Agent 响应路由到目标地。
#
# 投送目标优先级：
#   1. 明确指定的目标（如 "telegram:123456789"）
#   2. 平台主频道（如 "telegram" → 主频道）
#   3. 来源地（返回到创建任务的地方）
#   4. 本地文件（始终保存到文件）
#
# 关键特性：
#   - 支持多平台投送（Telegram、Discord、Slack 等）
#   - 自动截断超长消息（适配各平台限制）
#   - 消息分块传输（支持长消息的平台）
#   - 静默处理（过滤无关紧要的日志/输出）
#
...
#         → delivery.py:route_message()
#             → 解析目标地址
#             → 选择投送适配器
#             → 发送消息
#
# 消息大小限制：
#   - Telegram: 4096 字符（API 硬限制）
#   - 本模块截断阈值: 4000 字符（留出页脚空间）
# =============================================================================
